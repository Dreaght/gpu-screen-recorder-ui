...
namespace gsr {
        ...
        status_text("", get_theme().body_font_desc.c_str())
    {
        ...
        seek_worker_thread = std::thread([this]() { seek_worker_loop(); });
    ...
    VideoPlayer::~VideoPlayer() {
        {
            ...
            stop_seek_worker = true;
        }
        seek_cv.notify_one();
        if(seek_worker_thread.joinable())
            seek_worker_thread.join();
    ...
    bool VideoPlayer::on_event(mgl::Event &event, mgl::Window&, mgl::vec2f offset) {
        ...
        if(event.type == mgl::Event::MouseButtonReleased && event.mouse_button.button == mgl::Mouse::Left && dragging_seekbar) {
            if(dragging_seek_position_valid) {
                ...
                queue_seek_request(dragging_seek_position_ms, true);
            }
    ...
    void VideoPlayer::set_video_path(std::string video_path) {
        ...
        dragging_seek_position_valid = false;
        {
            ...
            pending_seek_position_ms = 0;
            pending_seek_generation = video_generation;
        }
    ...
    bool VideoPlayer::play() {
        const bool should_restart_from_beginning = cached_eof_reached && !displayed_seek_position_valid;
    ...
    bool VideoPlayer::toggle_pause() {
        return cached_pause ? play() : libmpv.pause();
    }
    ...
    void VideoPlayer::ensure_video_loaded() {
        if(!pending_video_path.empty() && cached_file_loaded) {
    ...
    void VideoPlayer::update_drag_seek(mgl::vec2f draw_pos, mgl::vec2f item_size, mgl::vec2f mouse_pos) {
        const int64_t duration_ms = cached_duration_ms;
        if(duration_ms <= 0) {
            dragging_seek_position_valid = false;
            return;
        }
    ...
    void VideoPlayer::process_pending_seek_display_state() {
        if(displayed_seek_position_valid && !dragging_seekbar) {
            ...
            const int64_t diff_ms = std::llabs(cached_position_ms - displayed_seek_position_ms);
    ...
    void VideoPlayer::refresh_cached_player_state() {
        cached_file_loaded = libmpv.is_file_loaded();
        cached_pause = libmpv.get_pause();
        cached_eof_reached = libmpv.is_eof_reached();

        if(!dragging_seekbar) {
            cached_duration_ms = libmpv.get_duration_ms();
            cached_position_ms = libmpv.get_position_ms();
        }
    }

    void VideoPlayer::queue_seek_request(int64_t position_ms, bool exact) {
        {
            std::lock_guard<std::mutex> lock(seek_mutex);
            pending_seek_request = true;
            pending_seek_position_ms = position_ms;
            pending_seek_exact = pending_seek_exact || exact;
            pending_seek_generation = video_generation;
        }
        seek_cv.notify_one();
    }

    void VideoPlayer::seek_worker_loop() {
        for(;;) {
            ...
            uint64_t seek_generation = 0;
            {
                ...
                seek_cv.wait(lock, [this]() { return stop_seek_worker || pending_seek_request; });
                if(stop_seek_worker)
                    break;

                seek_position_ms = pending_seek_position_ms;
                seek_exact = pending_seek_exact;
                seek_generation = pending_seek_generation;
                pending_seek_request = false;
                pending_seek_exact = false;
            }

            if(seek_generation != video_generation)
                continue;

            libmpv.seek_to_ms(seek_position_ms, seek_exact);
        }
    }
    ...
    void VideoPlayer::draw_video_surface(mgl::Window &window, mgl::vec2f draw_pos, mgl::vec2f item_size) {
        ...
        if(!libmpv.is_available() || !cached_file_loaded)
            return;
    ...
    void VideoPlayer::draw_overlay_controls(mgl::Window &window, mgl::vec2f draw_pos, mgl::vec2f item_size, bool controls_visible) {
        ...
        mgl::Sprite play_pause_icon(cached_pause ? &get_theme().play_texture : &get_theme().pause_texture);
        ...
        const float progress = duration_ms > 0
            ? ((cached_eof_reached && !dragging_seekbar && !using_displayed_scrub_position)
                ? 1.0f
